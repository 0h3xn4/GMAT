...
void CCSDSEMReader::Initialize()
{
   ...
   if (originator == "")
   {
      std::string warn = "WARNING : Ephemeris message file \"";
      warn += emFile + "\".  ";
      warn += "ORIGINATOR is missing or invalid.\n";
   ...
   if (creationDate == "")
   {
      std::string warn = "WARNING : Ephemeris message file \"";
      warn += emFile + "\".  ";
      warn += "CREATION_DATE is missing or invalid.\n";
      MessageInterface::ShowMessage(warn);
   }
...
// Read next non-empy line from input
// Return true if sucessful
// -----------------------------------------------------------------------------
bool CCSDSEMReader::ReadLine(std::string &line)
{
   std::string s;
   line = "";

   // Search until non-empy line or EOF
   while (true)
   {
      if (!ephFile || ephFile.eof())
         return false;
